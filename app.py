...
# --- 2. BARRA LATERAL (SIDEBAR): REGISTRO DE NUEVAS VENTAS ---
...
#     # Este es el número de cuotas que el usuario decide
...
#     tipo_pago = st.selectbox("Tipo de Pago*", ["Efectivo", "Transferencia", "Tarjeta", "Giro"])
#     estado_input = st.selectbox("Estado*", ["Activo", "Cancelado"])
    
#     btn_crear = st.form_submit_button("Guardar Venta y Generar Cuotas")
    
#     if btn_crear:
#         campos_validos = [producto.strip() != "", cliente.strip() != "", precio > 0, cantidad_cuotas > 0]

#         if all(campos_validos):
#             try:
#                 with engine.begin() as conn:
#                     # 1. Insertamos la venta
#                     # ASIGNACIÓN: 'cantidad_cuotas' ahora va a la columna 'total_cuota'
#                     sql_venta = text("""
#                         INSERT INTO ventas (
#                             producto, cliente, precio, total_cuota, monto_cuota, 
#                             comision, tipo_pago, estado, fecha_creacion
#                         ) VALUES (
#                             :p, :c, :pre, :tcuo, :mcuo, :com, :tpago, :est, :fecha
#                         ) RETURNING id_producto
#                     """)
                    
#                     result = conn.execute(sql_venta, {
#                         "p": producto, 
#                         "c": cliente, 
#                         "pre": precio, 
#                         "tcuo": cantidad_cuotas, # <--- Aquí se guarda la cantidad (ej: 3)
...
#                     # 2. Generamos automáticamente las cuotas en detalle_ventas
#                     sql_cuota = text("""
#                         INSERT INTO detalle_ventas (
#                             producto_id, item_cuota, monto_cuota, saldo_cuota, estado
#                         ) VALUES (:id_p, :item, :monto, :saldo, 'Activo')
#                     """)
                    
#                     # Usamos 'cantidad_cuotas' para el bucle
...
#                 st.sidebar.success(f"✅ Venta #{nuevo_id} creada con {cantidad_cuotas} cuotas en total_cuota.")
#                 st.rerun()
                
#             except Exception as e:
#                 st.sidebar.error(f"Error al guardar: {e}")
#         else:
#             st.sidebar.warning("⚠️ Complete los campos obligatorios.")  
